...
class MenuLib {
	...
	public function get_perfiles_asig_noasig($menu_id){
		...
		foreach($perfiles as $perfil){
			...
			if($query->num_rows > 0){
				$asig[] = array($perfil->id,$perfil->name,$menu_id);
			}
			else{
				$no_asig[] = array($perfil->id,$perfil->name,$menu_id);
			}
			
		}
		
		return array($no_asig,$asig);
	}
	
	public function findByControlador($controlador,$accion){
		$this->CI->db->where("controlador",$controlador);
		$this->CI->db->where("accion",$accion);
