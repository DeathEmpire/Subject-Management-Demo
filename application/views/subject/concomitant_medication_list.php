		...
		<tbody>
			...
			?>
				<tr>
					...
					<td><?= $v->frequency; ?></td>
					<td><?= $v->route; ?></td>
					<td><?= (($v->start_date != '0000-00-00') ? date('d/m/Y', strtotime($v->start_date)) : '' ); ?></td>
					<td><?= (($v->on_going == 1) ? 'Si' : 'No'); ?></td>
					<td><?= (($v->end_date != '0000-00-00') ? date('d/m/Y', strtotime($v->end_date)) : '' ); ?></td>					
					
					<td><?= anchor('subject/concomitant_medication_show/'. $subject->id .'/'.$v->id,'Ver', array('class'=>'btn btn-default')); ?></td>
				</tr>
...
<?php
		...
		echo "<div style='text-align:center;font-weight:bold;'>El sujeto no tiene agregado formulario de medicación concomitante.</div>";
	}
?>